#!/usr/bin/env python3
"""
Medical QA Evaluation Script
This script evaluates a locally fine-tuned T5 model on a medical QA task, using FAISS retrieval for context.
It prints the retrieved context for debugging and calculates BLEU scores.
"""
...
from peft import PeftModel, PeftConfig  # Import PEFT for LoRA fine-tuned models
...
class MedicalQAEvaluator:
    def __init__(
        ...
        """
        Initialize the Medical QA Evaluator with a locally fine-tuned model.
    ...
    def _load_local_t5_model(self):
        """
        Load the T5 model and tokenizer from local directory.
        """
        try:
            logger.info(f"Loading T5 model from local directory: {self.model_dir}")
            
            # Check if the model is a LoRA model (has adapter_config.json)
            is_lora_model = os.path.exists(os.path.join(self.model_dir, "adapter_config.json"))
            base_model_name = "google/flan-t5-base"  # Default base model
            
            # Load tokenizer
            if os.path.exists(os.path.join(self.model_dir, "tokenizer_config.json")):
                self.t5_tokenizer = T5Tokenizer.from_pretrained(self.model_dir)
                logger.info("Loaded tokenizer from local directory")
            else:
                # If tokenizer is not in the directory, load from base model
                self.t5_tokenizer = T5Tokenizer.from_pretrained(base_model_name)
                logger.info(f"Tokenizer not found in {self.model_dir}, loaded from {base_model_name}")
            
            # Load model
            if is_lora_model:
                logger.info("Detected LoRA fine-tuned model")
                # Load base model
                ...
                logger.info(f"Loaded LoRA adapter from {self.model_dir}")
            else:
                # Load full model
                ...
                logger.info(f"Loaded full model from {self.model_dir}")
    ...
    def embed_questions(self, questions: list) -> np.ndarray:
        """
    ...
    def retrieve_similar_qa_pairs(self, questions: list) -> list:
        """
    ...
    def format_inputs(self, questions: list, all_retrieved_qa: list) -> list:
        """
    ...
    def generate_answers(self, formatted_inputs: list) -> list:
        """
    ...
    def calculate_bleu(self, generated_answers: list, reference_answers: list) -> dict:
        """
...
def main():    
    parser = argparse.ArgumentParser(description="Evaluate fine-tuned T5 model on medical QA dataset")
