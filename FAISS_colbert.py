...
import torch
import numpy as np
from transformers import T5ForConditionalGeneration, T5Tokenizer, AutoTokenizer, AutoModel, AutoModelForSequenceClassification
...
class MedicalQAEvaluator:
    def __init__(
        ...
        colbert_model_name: str = "castorini/colbert-trained-msmarco",
        ...
        """
        Initialize the Medical QA Evaluator with ColBERT reranking.
        
        Args:
            t5_model_name: HuggingFace model name for T5
            colbert_model_name: HuggingFace model name for ColBERT reranker
    ...
    def _load_colbert_model(self):
        """Load the ColBERT model for reranking."""
        try:
            logger.info(f"Loading ColBERT model: {self.colbert_model_name}")
            
            # Load tokenizer and model for ColBERT
            self.colbert_tokenizer = AutoTokenizer.from_pretrained(self.colbert_model_name)
            
            # Load as a sequence classification model
            self.colbert_model = AutoModelForSequenceClassification.from_pretrained(
                self.colbert_model_name,
                torch_dtype=torch.float16 if torch.cuda.is_available() else torch.float32
            ).to(self.device)
            
            self.colbert_model.eval()  # Set to evaluation mode
    ...
    def colbert_rerank(self, questions: List[str], retrieved_qa_list: List[List[Dict]]) -> List[List[Dict]]:
        """
        Rerank retrieved QA pairs using ColBERT.
        ...
        for i, (question, retrieved_qa) in enumerate(zip(questions, retrieved_qa_list)):
            ...
            for qa in retrieved_qa:
                ...
                candidates.append(qa['question'] + " " + qa['answer'])
            
            # Get reranked scores in batches
            scores = []
            ...
            for j in range(0, len(candidates), batch_size):
                ...
                # Tokenize inputs for ColBERT
                inputs = []
                for candidate in batch_candidates:
                    # Tokenize query and document pairs
                    encoded = self.colbert_tokenizer.encode_plus(
                        question,
                        candidate,
                        max_length=self.max_input_length,
                        truncation=True,
                    ...
                    inputs.append({
                        ...
                        "token_type_ids": encoded.get("token_type_ids", None)  # Some models don't return this
                    })
                
                # Prepare batch tensors
                batch_input_ids = torch.cat([x["input_ids"] for x in inputs]).to(self.device)
                batch_attention_mask = torch.cat([x["attention_mask"] for x in inputs]).to(self.device)
                
                # Handle models with or without token_type_ids
                batch_token_type_ids = None
                if inputs[0]["token_type_ids"] is not None:
                    batch_token_type_ids = torch.cat([x["token_type_ids"] for x in inputs]).to(self.device)
                
                # Get scores from ColBERT model
                with torch.no_grad():
                    model_inputs = {
                    ...
                    if batch_token_type_ids is not None:
                        model_inputs["token_type_ids"] = batch_token_type_ids
                        
                    outputs = self.colbert_model(**model_inputs)
                    
                    # Extract scores - model may return logits in different formats
                    if hasattr(outputs, "logits") and outputs.logits.dim() == 2:
                        batch_scores = outputs.logits[:, 1].cpu().numpy()  # For binary classification, take positive class
                    elif hasattr(outputs, "logits") and outputs.logits.dim() == 1:
                        batch_scores = outputs.logits.cpu().numpy()  # Direct scores
                    else:
                        batch_scores = outputs[0].squeeze(-1).cpu().numpy()  # Fall back to first output
                    
                    scores.extend(batch_scores.tolist())
            
            # Sort QA pairs by score and take top_k
            scored_qa = [(score, qa, idx) for idx, (score, qa) in enumerate(zip(scores, retrieved_qa))]
            # Sort by score (primary) and original index (secondary)
            scored_qa.sort(key=lambda x: (x[0], -x[2]), reverse=True)
            # Extract just the qa_pairs in sorted order
    ...
    def retrieve_and_rerank_qa_pairs(self, questions: List[str]) -> List[List[Dict]]:
        ...
        reranked_qa_list = self.colbert_rerank(questions, retrieved_qa_list)
        
        # Log some examples after reranking
        if len(questions) > 0 and len(reranked_qa_list[0]) > 0:
            logger.info("\n" + "="*50)
            logger.info(f"AFTER COLBERT RERANKING:")
            logger.info(f"Query: {questions[0]}")
    ...
    def evaluate(self, output_file: str = 'evaluation_results.json', sample_limit: int = 5000):
        """
        Run the full evaluation pipeline with ColBERT reranking.
        
        Args:
            output_file: Path to save the evaluation results
            sample_limit: Maximum number of test samples to evaluate
        """
        logger.info("Starting evaluation with ColBERT reranking")
        ...
        for i in tqdm(range(0, len(questions), batch_size), desc="Generating answers"):
            ...
            for j in range(len(batch_questions)):
                ...
                logger.info("RETRIEVED QA PAIRS (after ColBERT reranking):")
...
def main():
    """Main function to run the evaluation with ColBERT reranking."""
    # First, check for required packages
    required_packages = ['rouge_score', 'bert_score', 'evaluate']
    missing_packages = []
    ...
    if missing_packages:
        print("WARNING: The following packages are required for all evaluation metrics:")
        for package in missing_packages:
            print(f"  - {package}")
        print("\nTo install missing packages, run:")
        print("pip install rouge-score bert-score evaluate")
        print("\nContinuing with available metrics...\n")
    
    parser = argparse.ArgumentParser(description="Evaluate T5 model on medical QA dataset with ColBERT reranking")
    parser.add_argument("--t5_model_name", type=str, default="google/flan-t5-base", 
                        help="HuggingFace T5 model name")
    parser.add_argument("--colbert_model_name", type=str, default="castorini/colbert-trained-msmarco", 
                        help="HuggingFace model name for ColBERT reranker")
    ...
    parser.add_argument("--output_file", type=str, default="new_output/T5_FAISS_Colbert_evaluation_results.json", 
    ...
    parser.add_argument("--metrics", type=str, default="all",
                        help="Comma-separated list of metrics to use: bleu,rouge,bertscore,meteor,all")
